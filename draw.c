...
void scanline_convert( struct matrix *polygons, int i, screen s, zbuffer zb, color c) {
  double tX, tY, tZ, mX, mY, mZ, bX, bY, bZ, dx0, dz0, dx1, dz1, x0, x1, x2, y0, y1, y2, z0, z1, z2;
  ...
  if (mY == bY) {
    ...
    dx1 = (mX - bX) / (mY - bY);
    dz1 = (mZ - bZ) / (mY - bY); }
  for (i = 0; i < mY - bY; i++) draw_line(bX + (i * dx0), bY + i, bZ + (i * dz0), bX + (i * dx1), bY + i, bZ + (i * dz1), s, zb, c);
  ...
  if (tY == mY) {
    ...
    dx1 = (tX - mX) / (tY - mY);
    dz1 = (tZ - mZ) / (tY - mY); }
  for (i = 0; i < tY - mY; i++) draw_line(bX + ((i + mY - bY) * dx0), mY + i, bZ + ((i + mY - bY) * dz0), mX + (i * dx1), mY + i, mZ + (i * dz1), s, zb, c);
}
...
void draw_polygons( struct matrix *polygons, screen s, zbuffer zb, color c ) {
  ...
  for (point=0; point < polygons->lastcol-2; point+=3) {
    ...
    if ( normal[2] > 0 ) {
      ...
      //printf("polygon %d\n", point);
      if (c.red <= 50) c.red = (c.red + 2700) % 255;
      if (c.green <= 50) c.red = (c.red + 1722) % 255;
      if (c.blue <= 50) c.red = (c.red + 1821) % 255;
